...
def read_multiple_csv(path, col = None, parse_dates = None):
	...
	if parse_dates == None:
		...
		if col is None:
			df = pd.concat([pd.read_csv(f, parse_dates = ['date'] ) for f in tqdm(sorted(glob(path+'/*')))])
		else:
			df = pd.concat([pd.read_csv(f, parse_dates = ['date'] )[col] for f in tqdm(sorted(glob(path+'/*')))])
	return df

def reduce_memory(df, ix_start=0):
    df.fillna(-1, inplace=True)
    if df.shape[0] <= 9999:
        df_ = df.sample(df.shape[0], random_state=71) # just for small sample
    else:
